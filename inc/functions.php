<?php
	...
	function _hw_io_apply_orientation( $filePath, $orientation = 1, $exifData = [] ){
	...
	function _hw_io_ajax(){
		...
		if( $_POST['do'] == 'reorient' ){
			...
			$R[ $image['post']->ID ] = [ 'name' => $image['post']->post_title ];
	...
	function _hw_io_getOrientFromImagePost( $postOrId, $returnOnlyOrientation = true ){
		...
		return $returnOnlyOrientation ? $metaData['image_meta']['orientation'] : [
